...
spidmap_file <- "L:/Lab/NHEERL_Mundy/Project - DNT Test Set 2021 NTP/Coded Plate Map - Plate No. 4001207126 DNT HEI EPA SHIP CHEM14214 MRI3110.xlsx" # path to file that maps treatment names to sample IDs (optional)
spid_sheet <- 1 # desired sheet in spidmap_file (can be numeric or the sheet name) (optional)

project.input.dir <- "L:/Lab/NHEERL_MEA/Project - DNT Test Set 2021 NTP" # main folder of experimental data for project
scripts.dir <- "C:/Users/acarpe01/Documents/local_repos/CCTE_Shafer_MEA_dev_pre-processing_scripts/R" # directory containing the pre-processing scripts for the NFA
root.output.dir <- "C:/Users/acarpe01/Documents/local_repos/data_process_prac" # main directory where the output should be saved
assay_component_map_filename <- "L:/Lab/NHEERL_MEA/CCTE_Shafer pre-process for TCPL/script_repos/CCTE_Shafer_MEA_dev_pre-processing_scripts/mea_nfa_component_name_map.csv"
...
dat[apid == '20210929_MW75-9308' & treatment == '7126 A3', treatment := '7126 B3']
```
...
* stock_conc_units = units of the stock concentration
...
# Add the file name
spidmap[, 'spidmap_file' := spidmap_file]
...
# the blind code is not actually the sample ID, but we can treat it like it is for this run_me practice file
setnames(spidmap, old = 'Blind-Code', new = 'spid')
...
# **For this example only** - restrict the dat to specific treatments
dat <- dat[grepl('7126',treatment) | treatment %in% c("DMSO","Bisphenol A","Acetamenophin")]
...
dat <- merge(dat, spidmap[, .(treatment, spid, stock_conc, stock_conc_units, spidmap_file)],
...
# **For this example only**
dat[spid %in% c("Acetamenophin","Bisphenol A"), wllt := 'x']
...
# Therefore, I think that any variability in the concentrations due to differences in data processing of decimal places/rounding/stock concentrations in different input files is negligible if it is less that 0.005 log10-uM (2 orders of magnitude lower).
...
**Specific note for this example with the DNT NTP 2021 data:**

All of these treatments were intentionally retested at a lower concentration, so all of these are fine.

Below is an example of updating my_anticipated conc's based on information from e.g., lab notebooks for the affected treatments, then re-checking the concentration agreement. (Don't worry about the specifics of this too much)


```{r}
dat[conc_log10_diff_abs >= 0.005, .N, by = .(treatment, culture_date)][order(culture_date)]

# Update the top_conc for each treatment-culture_date manually
# (based on lab notebook)
# there may be a less onerous way to do this...

# These treatment-cultures tested at 1000-x lower conc
dat[treatment == "7126 A12" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 B11" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 C11" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 D4" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 D5" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 F1" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 F8" & culture_date == "20220601", top_conc := stock_conc/1000]
dat[treatment == "7126 H10" & culture_date == "20220601", top_conc := stock_conc/1000]

# These treatment-cultures tested at 100-x lower conc
dat[treatment == "7126 D7" & culture_date == "20220601", top_conc := stock_conc/100]


# Note from lab notebook on 7126 A4, 20210915:
...
# Update the expected concentrations accordingly:

# Update top conc:
dat[treatment == '7126 A4' & culture_date == '20210915', top_conc := (stock_conc/100)*15/(35+15)] # based on above notes, copied from lab notebook
...
If no conc's differ from my_anticipated_conc by more than 0.005, set all conc's to my_anticipated_conc for consistency
...
# **for this example only**
dat[spid %in% c('Acetamenophin',' Bisphenol A'), conc := conc_srcf]
```
...
# For test wells, if the stock_conc_units are mM, then the units for the MEA and cytotoxicity data are usually uM (micromolar)
dat[wllt == 't' & stock_conc_units == 'mM', units := 'uM']
...
dat[grepl("LDH",acsn), assay := 'LDH']
dat[grepl("AB",acsn), assay := 'AB']
...
# In these cases, if there are any NA rvals with wllq == 1, that indicates there was a problem in the data files, data collection, and/or pre-processing
# These instances warrant further investigations
dat[(is.na(rval) | is.infinite(rval)) & wllq == 1 & !grepl('DIV',acsn), .N, by = .(acsn)]
stopifnot(nrow(dat[is.na(rval) & wllq == 1 & !grepl('DIV',acsn)]) == 0)

# Note NA's in the DIV-specific parameters
# These can be left as NA, but just note the occurrence
dat[(is.na(rval) | is.infinite(rval)) & wllq == 1 & grepl('DIV',acsn), .N, by = .(acsn)]
...
```{r}
dat[treatment == '7126 B1', .N, by = .(culture_date, wllq, wllq_notes)]
```
...
```{r}
dat[, acsn_short := sub("CCTE_Shafer_MEA_dev_","",acsn)]
```
...
dat[wllq == 1, .(min = format(min(rval,na.rm=T),digits=2,scientific = F), 
                 ...
                 num_NA = sum(is.na(rval))), by = .(acsn_short)][order(acsn_short)]
```
