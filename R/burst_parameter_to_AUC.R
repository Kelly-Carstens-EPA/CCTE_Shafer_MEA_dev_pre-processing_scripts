...
burst_parameter_to_AUC <- function(project.output.dir, 
                                   project_name, 
                                   get_files_under_project.output.dir = TRUE,
                                   use_divs = c(5,7,9,12),
  ...
  # Set output file name and folder
  output.filename <- paste0(project_name,"_AUC.csv")
  ...
  if(get_files_under_project.output.dir) {
    # get files with the 16 parameters for all culture dates
    parameter_data_files <- list.files(path = file.path(project.output.dir, "prepared_data"), pattern = "\\.csv$", full.names = TRUE, recursive = FALSE)
    cat("Got",length(parameter_data_files),"parameter csv files from",file.path(project.output.dir, "prepared_data"),"\n")
    
    # get files with mi data for all culture dates
    mi_data_files <- list.files(path = file.path(project.output.dir, "All_MI"), pattern = "\\.csv$", full.names = TRUE, recursive = FALSE)
    cat("Got",length(mi_data_files),"MI csv files from",file.path(project.output.dir, "All_MI"),"\n")
    
  } else {
    # get files with the 16 parameters for all culture dates
    parameter_data_files <- choose.files(default = project.output.dir, caption = "Select all files containing activity parameter values to calculate AUC")
    cat("Got",length(parameter_data_files),"parameter data files.\n")
    # get files with mi data for all culture dates
    mi_data_files <- choose.files(default = project.output.dir, caption = "Select all files containing MI data to calculate AUC")
  ...
  # update wllq -------------------------------------------------------------------------
  cat("\nUpdating wllq...\n")
  wllq_info <- as.data.table(read.csv(file.path(project.output.dir, "wells_with_well_quality_zero.csv"), colClasses = c(rep("character",4),"numeric",rep("character",2)), stringsAsFactors = F))
  ...
  write.csv(all_data, file = file.path(project.output.dir,"output",paste0(project_name,"_parameters_by_DIV.csv")), row.names = FALSE)
  ...
  write.csv(sum_table, file.path(project.output.dir, "output", output.filename), row.names = FALSE)
  
  rm(list = c("all_data","all_data_split","parameter_data","mi_data","sum_table"))
  cat("\n", output.filename," is ready\n",sep = "")
