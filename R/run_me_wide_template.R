...
# USER INPUT
###################################################################################
dataset_title <- "testpipeline2020" # the name for the current dataset, e.g. "name2020" (this should match the name of the output folder)
...
auc_table <- read.csv(file.path(root_output_dir, dataset_title, "output", paste0(dataset_title,"_AUC.csv")), stringsAsFactors = F)
...
write.csv(auc_table_normalized_dn, file = file.path(root_output_dir, dataset_title, "output", paste0(dataset_title,"_AUC_normalized_down.csv")), row.names = FALSE)
...
write.csv(auc_table_normalized_up, file = file.path(root_output_dir, dataset_title, "output", paste0(dataset_title,"_AUC_normalized_up.csv")), row.names = FALSE)
...
rm(list = setdiff(ls(),c(grep('auc',ls(),val = T),'save_notes_graphs','scripts.dir','root_output_dir','dataset_title')))
