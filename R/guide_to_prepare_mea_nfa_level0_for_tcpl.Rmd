...
# R things I'd suggest Manasvinee learn

* = vs == vs <-
* grepl and grep
* subsetting a list with [] and either integers or boolean
* stringi, regex
* boolean**
...
* functions
* sapply, lapply
* data.table basics (e.g., :=, NULL, etc)
...
Note that the wllq notes entered in the first table ("[project_name]_well_quality_assigments_by_well.csv") will be applied from within the function parameter_values_to_AUC(). The wllq notes in the second table ("[project_name]_well_quality_assignments_by_compound_cndx_culture_date.csv") will be applied within the run_me RMD AFTER the treatment names and concentrations have been verified by you.
...
wllq.by.well.desc <- data.table(
  ...
  'Description' = c("Comma-separated list of which assays the well-quality affects, including mea, CTB (Cell Titer Blue), or LDH. (Note that the CTB assay may also be referred to as Alamar Blue)",
                    ...
                    "Day in vitro. If all DIV are affected, enter 'all'. (If any DIV other than 5, 7, 9, and 12 are affected, create a separate row in the well quality table for each DIV). If the affected assay is a cytotoxicity assay (LDH or CTB), this column will be ignored.",
...
In order to combine the parameter values from each DIV, we calculate the trapezoidal Area-Under-the-Curve (AUC) values for each parameter value in every well. This step performs that calculation, in addition to some other checks. The function parameter_values_to_AUC() takes the csv files that contain the 16 parameters and the mutual information for each DIV as input. Two output files are saved in the project.output.dir in a folder called 'output'. The first file, with the suffix _parameters_by_DIV, contains all of the input parameter data files. The second file, with the suffix _AUC, contains the AUC values with every data row corresponding to 1 well.

parameter_value_to_AUC() arguments:

* project_output_dir - folder where output should be saved
* project_name - project_name
* div_data_file_folders - folders that contain the .csv files with the parameter data by DIV (can include multiple folder, usually 1 for the 16-parameter files and one for the MI files). Should be the full path to the folders
* id.columns - columns in the input parameter data by DIV that should be interpreted as ID columns. Must include "DIV"
* dose_column_name - (optional) name of column in input data files containing the dose. This column will be converted to character to ensure that any minor discrepancies in the numeric dose value do not interfere with the data grouping for the AUC calculation
* use_divs = c(5,7,9,12),
* interpolate_diff_divs = TRUE,
* add_DIV2_values_of_0 - binary of whether a value of "0" should be added at DIV 2 for every well and parameter value. **If you want to be consistent with data produced by Shafer lab, this should be true**. Background: Earlier in the development of the NFA, recordings were made on DIV 2, 5, 7, 9, and 12. Eventually, because so little activity was observed on DIV 2, they stopped recording on DIV 2. When the script `burst_parameter_to_AUC.R` was made, the writer decided to add a point at (DIV= 2, value = 0) for every well and parameter value. This choice to impute a dummy data at DIV 2 has been the practice in the Shafer lab for all data that has been pipelined in TCPL as of July 2023.


**Notes**:

* the function parameter_values_to_AUC() is a newer version of the function burst_parameter_to_AUC(). Both functions are desired to perform the same task, but parameter_values_to_AUC() should be more flexible and easier to debug.
* Note that AFTER the _parameters_by_DIV.csv file is saved but BEFORE the AUC calculation, all NA parameter values are set to 0. This is done in order to have a data point at every DIV for the AUC calculation. However, this appropriateness of this practice is uncertain for some parameters.

* You may see the message "Some rows in  [project_name]_well_quality_assignments_by_well.csv  did not match any rows in dat:". If there are no NFA-related well quality notes in THIS well quality notes table, that is okay. However, if you were expected some fields to match, then check that the affected_assays column in the well quality notes table contains 'nfa'

To do:
* check that there are 4 (or len of use.dIVs) per grouping of by term (or not is essentially already done)
* confirm all same functionality! 
* address the "Fix" thing
